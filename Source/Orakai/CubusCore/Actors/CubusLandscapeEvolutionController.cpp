...
void ACubusLandscapeEvolutionController::EvolveLandscape()
{
	...
	UE_LOG(LogTemp, Display,
		   TEXT("Cubus landscape evolved: max |delta| %.2f m, mean |delta| %.2f m, max lowering %.2f m, max raising %.2f m, max incision "
				"%.2f m"),
		   Stats.MaximumAbsoluteElevationChangeM, Stats.MeanAbsoluteElevationChangeM, Stats.MaximumTerrainLoweringM,
		   Stats.MaximumTerrainRaisingM, Stats.MaximumStreamIncisionM);
	RebuildPreview();
}
...
void ACubusLandscapeEvolutionController::GenerateSolveAndEvolve()
{
	...
	UpdateDiagnostics(Stats);
	UE_LOG(LogTemp, Display,
		   TEXT("Cubus landscape generated + evolved: max |delta| %.2f m, mean |delta| %.2f m, max lowering %.2f m, max raising %.2f m, "
				"max incision %.2f m"),
		   Stats.MaximumAbsoluteElevationChangeM, Stats.MeanAbsoluteElevationChangeM, Stats.MaximumTerrainLoweringM,
		   Stats.MaximumTerrainRaisingM, Stats.MaximumStreamIncisionM);
	RebuildPreview();
		...
		TEXT("CUBUS LANDSCAPE\n") TEXT("World: %.0f x %.0f km\n") TEXT("Global DEM: %d x %d\n") TEXT("Global cell: %.2f m\n")
			TEXT("Preview: %s, %d x %d\n") TEXT("Displayed cell: %.2f m\n") TEXT("Window: %.2f km\n") TEXT("Center: (%.0f, %.0f) m\n")
				TEXT("Debug view: %s\n") TEXT("Land: %.1f%%   Mean slope: %.1f deg   Steep 35+: %.1f%%\n")
					TEXT("Max neighbour step: %.1f m\n") TEXT("Rivers: %d   Basins: %d\n") TEXT("Max incision: %.2f m\n")
						TEXT("Max |delta|: %.2f m"),
		WorldSizeMeters / 1000.0, WorldSizeMeters / 1000.0, GlobalResolution, GlobalResolution, GlobalCellSizeM, PreviewModeText,
		PreviewResolution, PreviewResolution, PreviewDisplayedCellSizeM, PreviewWindowSizeKm, PreviewActualCenterWorldMeters.X,
		PreviewActualCenterWorldMeters.Y, DebugViewText, LandPercent, MeanLandSlopeDegrees, SteepLandPercent, MaximumNeighbourStepM,
...
FLinearColor ACubusLandscapeEvolutionController::DebugColorForCell(const int32 Cell) const
{
	...
	case ECubusLandscapeDebugView::Province:
	{
		static const FLinearColor Colors[] = {FLinearColor(0.40f, 0.72f, 0.35f), FLinearColor(0.65f, 0.52f, 0.30f),
...
void ACubusLandscapeEvolutionController::RebuildPreview()
{
	...
	const double	 EffectiveVerticalScale =
		PreviewMode == ECubusLandscapePreviewMode::FullWorldOverview ? FMath::Max(8.0f, PreviewVerticalScale) : PreviewVerticalScale;
	const double PreviewHeightScale = PreviewWorldScale * EffectiveVerticalScale;
	...
	if (bNativeDetail)
	{
		...
		PreviewActualCenterWorldMeters =
			FVector2D(-HalfWorld + ActualCenterX * GlobalDem.CellSizeMeters, -HalfWorld + ActualCenterY * GlobalDem.CellSizeMeters);
	...
	UE_LOG(LogTemp, Display, TEXT("Cubus preview: %s, %.2f m/vertex, %.2f km window, center (%.0f, %.0f) m"),
		   bNativeDetail ? TEXT("Native Detail") : TEXT("Full World Overview"), PreviewDisplayedCellSizeM, PreviewWindowSizeKm,
		   PreviewActualCenterWorldMeters.X, PreviewActualCenterWorldMeters.Y);
}
