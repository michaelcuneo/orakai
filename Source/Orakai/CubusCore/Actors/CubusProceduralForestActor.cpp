...
{
void AppendSection(const FCubusTreeMeshSection& Source, const FTransform& Transform, TArray<FVector>& Vertices, TArray<int32>& Triangles,
				   TArray<FVector>& Normals, TArray<FVector2D>& UV0, TArray<FLinearColor>& VertexColors)
{
...
bool IsFarEnough(const FVector2D& Candidate, const TArray<FVector2D>& Existing, const float MinimumSpacingSquared)
{
...
void ACubusProceduralForestActor::RebuildForest()
{
	...
	for (int32 Attempt = 0; Attempt < MaximumAttempts && GeneratedTreeCount < DesiredCount; ++Attempt)
	{
		...
		if (!CubusProceduralForest::IsFarEnough(Candidate, AcceptedPositions, MinimumSpacingSquared))
		{
			continue;
		}
		...
		if (!FCubusProceduralTreeGenerator::BuildTree(*Species, TreeSeed, GeneratedTree))
		{
			continue;
		}
		...
		const FTransform TreeTransform(FRotator(0.0f, Yaw, 0.0f), LocalPosition, FVector(Scale));

		CubusProceduralForest::AppendSection(GeneratedTree.Bark, TreeTransform, BarkVertices, BarkTriangles, BarkNormals, BarkUV0,
											 BarkColors);

		if (GeneratedTree.Canopy.IsValid())
		{
			CubusProceduralForest::AppendSection(GeneratedTree.Canopy, TreeTransform, CanopyVertices, CanopyTriangles, CanopyNormals,
												 CanopyUV0, CanopyColors);
		}
	...
	{
		ForestMesh->CreateMeshSection_LinearColor(0, BarkVertices, BarkTriangles, BarkNormals, BarkUV0, BarkColors, EmptyTangents,
												  bGenerateCollision);
	...
	{
		ForestMesh->CreateMeshSection_LinearColor(1, CanopyVertices, CanopyTriangles, CanopyNormals, CanopyUV0, CanopyColors, EmptyTangents,
												  false);
		ForestMesh->SetMaterial(1, Species->CanopyMaterial);
	}

	ForestMesh->SetCollisionEnabled(bGenerateCollision ? ECollisionEnabled::QueryAndPhysics : ECollisionEnabled::NoCollision);
