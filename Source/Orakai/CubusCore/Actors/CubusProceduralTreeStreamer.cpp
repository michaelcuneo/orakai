...
void AppendSection(const FCubusTreeMeshSection& Source, const FTransform& Transform, FCubusTreeMeshSection& Target)
{
...
void CreateSection(UProceduralMeshComponent& Mesh, const int32 SectionIndex, const FCubusTreeMeshSection& Section, const bool bCollision)
{
	if (!Section.IsValid())
	{
		return;
	...
	Mesh.CreateMeshSection_LinearColor(SectionIndex, Section.Vertices, Section.Triangles, Section.Normals, Section.UV0,
									   Section.VertexColors, Tangents, bCollision);
}
} // namespace
...
bool ACubusProceduralTreeStreamer::IsChunkVisible(const ACubusVoxelVolumeActor& Chunk, const FVector& CameraLocation,
												  const bool bHasCamera) const
{
	// Disable player-centric chunk culling so streamed procedural trees remain visible.
	return true;
}
...
void ACubusProceduralTreeStreamer::RefreshVisibleChunks()
{
	...
	for (TActorIterator<ACubusVoxelVolumeActor> Iterator(World); Iterator; ++Iterator)
	{
		...
		if (!IsValid(Chunk) || Chunk->GetOwner() != BlockWorld || !IsChunkVisible(*Chunk, CameraLocation, bHasCamera))
		{
			continue;
		}
...
void ACubusProceduralTreeStreamer::BuildChunkTrees(ACubusVoxelVolumeActor& Chunk, const uint32 Signature)
{
	const FCubusBlockChunkData* ChunkData = Chunk.GetChunkData();
	if (ChunkData == nullptr)
	{
	...
	if (!IsValid(Entry.Mesh))
	{
		Entry.Mesh = NewObject<UProceduralMeshComponent>(
			this, *FString::Printf(TEXT("ProceduralTrees_%d_%d_%d"), ChunkCoordinate.X, ChunkCoordinate.Y, ChunkCoordinate.Z));
		Entry.Mesh->SetupAttachment(Root);
		Entry.Mesh->RegisterComponent();
		Entry.Mesh->SetMobility(EComponentMobility::Movable);
		Entry.Mesh->SetCollisionEnabled(bGenerateCollision ? ECollisionEnabled::QueryAndPhysics : ECollisionEnabled::NoCollision);
	...
	for (const FCubusVegetationInstance& Instance : ChunkData->GetVegetationInstances())
	{
		...
		const uint32 InstanceHash =
			HashCombineFast(GetTypeHash(Instance.WorldVoxel), HashCombineFast(GetTypeHash(Instance.TypeId), GetTypeHash(GenerationSeed)));
		...
		const FTransform WorldTreeTransform(FQuat::Identity, WorldLocation, FVector(Scale));
