    ...
    if (bPublishedVegetationBudgetSaturated && bHasLastFullVegetationBuildCameraLocation)
    {
        ...
        if (IsValid(PlayerPawn))
        {
            ...
            bStreamingRecenterRequired =
                FVector::DistSquared(StreamingLocation, LastFullVegetationBuildCameraLocation) >=
...
void ACubusWorldVegetationActor::UpdateDynamicWindBridge()
{
    ...
    if (
        ...
        FMath::IsNearlyEqual(WindIntensity, LastBridgedWindIntensity, 0.001f)
    )
    {
        return;
    }
    ...
    const float GlobalFoliageWindSpeed =
        ...
        FMath::Pow(NormalizedUdwWind, FMath::Max(0.1f, GlobalFoliageWindResponseExponent));
    const FVector GlobalFoliageWindDirection =
        WindDirection.RotateAngleAxis(
            FMath::Clamp(GlobalFoliageWindDirectionYawOffset, -180.0f, 180.0f),
            FVector::UpVector
        ).GetSafeNormal();
    ...
    if (Now - LastBridgeDebugLogTime >= 2.0)
    {
        UE_LOG(
            ...
            UpdatedCollectionCount,
            IsValid(CachedDynamicWindCollection) ? *CachedDynamicWindCollection->GetName() : TEXT("None"),
            WindIntensity,
            WindDirection.X,
            WindDirection.Y,
            WindDirection.Z
        ...
        if (bBridgeUdwToGlobalFoliageActor)
        {
            UE_LOG(
                ...
                Display,
                TEXT(
                    "Cubus global foliage wind sync: actor=%s updatedProps=%d uds=%.3f mappedSpeed=%.3f exponent=%.2f udsYaw=%.1f "
    ...
    for (const auto& Pair : RegisteredChunks)
    {
        ...
        for (const FCubusVegetationInstance& Instance : Chunk->GetChunkData()->GetVegetationInstances())
        {
            ...
            const FVector Target =
                Resolved.Location + FVector::UpVector * SafeVoxelSize * 3.0f * Resolved.Scale;
...
bool ACubusWorldVegetationActor::EnsureFarVegetationProxyAssets(const bool bSaveGeneratedAssets)
{
    ...
    for (int32 SpeciesIndex = 0; SpeciesIndex < SpeciesCatalog.Num(); ++SpeciesIndex)
    {
        ...
        for (int32 StageIndex = 0; StageIndex < StageCount; ++StageIndex)
        {
            ...
            {
                GeneratedMesh = UAnimToTextureBPLibrary::ConvertSkeletalMeshToStaticMesh(
                    SourceSkeletalMesh,
                    PackageName,
                    0
                );
                ...
                if (IsValid(GeneratedMesh))
                {
                    ...
                    UE_LOG(
                        LogTemp,
                        Display,
                        TEXT("Cubus far proxy baked: species=%s stage=%d source=%s proxy=%s"),
                        *SpeciesToken,
                        StageIndex,
...
void ACubusWorldVegetationActor::RefreshFarVegetationBatches()
{
    ...
    for (int32 SpeciesIndex = 0; SpeciesIndex < SpeciesCatalog.Num(); ++SpeciesIndex)
    {
        ...
        for (int32 StageIndex = 0; StageIndex < Entry.GrowthStageMeshes.Num(); ++StageIndex)
        {
            ...
            int64 FarBatchKey = FCubusVegetationRenderer::MakePrimaryBatchKey(
                SpeciesIndex,
                StageIndex
            );
            ...
            else if (Cast<USkeletalMesh>(SourceAsset) != nullptr)
            {
                ...
                FarBatchKey = FCubusVegetationRenderer::MakeStaticFallbackBatchKey(
                    SpeciesIndex,
                    StageIndex
                );
                ...
                if (!IsValid(FarStaticMesh))
                {
                    ...
                    UE_LOG(
                        ...
                        Warning,
                        TEXT(
                            "Cubus far vegetation proxy missing: species=%s stage=%d path=%s. "
            ...
            const FName ComponentName(
                *FString::Printf(
                    ...
                    static_cast<uint64>(FarBatchKey)
                )
            );
    ...
    UE_LOG(
        ...
        Display,
        TEXT(
            "Cubus far vegetation batches: primaryStatic=%d generatedProxy=%d "
...
void ACubusWorldVegetationActor::RebuildWorldVegetation()
{
    ...
    if (bHasStreamingOrigin && bHasLastFullVegetationBuildCameraLocation)
    {
        const double RecenterDistance =
            static_cast<double>(FMath::Max(100.0f, VegetationRecenterDistance));
    ...
    )
    {
        ...
        PublishedPlacementHash = static_cast<int64>(
            CalculateVegetationSignatureMapHash(CurrentChunkVegetationSignatures)
        );
        PublishedVegetationSettingsHash = CurrentVegetationSettingsHash;
    ...
    for (const FIntVector& ChunkCoordinate : VegetationChunkCoordinates)
    {
        ...
        const float SafeVoxelSize = FMath::Max(1.0f, Chunk->GetVoxelSize());
        const double ChunkHalfWorldExtent =
            static_cast<double>(Cubus::ChunkSize) * static_cast<double>(SafeVoxelSize) * 0.5;
        ...
        for (const FCubusVegetationInstance& Instance : ChunkData->GetVegetationInstances())
        {
            ...
            if (
                ...
                (WorldLocation.Z < PruneMinWorldZ || WorldLocation.Z > PruneMaxWorldZ)
            )
            {
                continue;
            }
            ...
            const float SurfaceSlopeDegrees = FMath::RadiansToDegrees(
                FMath::Acos(
                    FMath::Clamp(
                        FVector::DotProduct(SurfaceNormal, FVector::UpVector),
                        -1.0f,
                        1.0f
                    )
                )
            ...
            const int32 SpeciesIndex = VegetationCatalog.SelectSpeciesIndex(
                Instance,
                SpeciesCatalog,
            ...
            const int64 PrimaryBatchKey = FCubusVegetationRenderer::MakePrimaryBatchKey(
                SpeciesIndex,
                GrowthStage
            );
            ...
            if (
                !CatalogGrassBatchComponents.Contains(TargetBatchKey) &&
                !CatalogStaticBatchComponents.Contains(TargetBatchKey) &&
            ...
            {
                const int32 RemainingInstanceBudget = PlantLimit == MAX_int32
                    ? MAX_int32
                    : FMath::Max(0, PlantLimit - RenderedInstanceBudgetCount);
                ...
                const int32 GrassCount = FMath::Min(
                ...
                {
                    const uint32 ScatterHash = HashCombineFast(
                    ...
                    const float SafeGrassScatterRadius = FMath::Min(
                        GrassScatterRadius,
                        SafeVoxelSize * 0.42f
                    );
                    ...
                    if (FMath::Abs(SurfaceNormal.Z) > 0.01f)
                    {
                        GrassWorldLocation.Z =
                            FinalLocation.Z -
                            (SurfaceNormal.X * ScatterOffset.X + SurfaceNormal.Y * ScatterOffset.Y) /
                            SurfaceNormal.Z;
                    }
                    ...
                    const FTransform GrassWorldTransform(
                        FRotator(0.0f, GrassYaw, 0.0f),
                        GrassWorldLocation,
                        FVector(GrassScale)
                    );

                    BatchTransforms.Add(
                        GrassWorldTransform.GetRelativeTransform(GetActorTransform())
                    );
                }
    ...
    const int32 HeroComponentLimit =
        bEnableHeroSkeletalWindMode && MaxHeroSkeletalWindComponents > 0
    ...
    for (const TPair<int64, TObjectPtr<UInstancedSkinnedMeshComponent>>& Pair : CatalogSkeletalBatchComponents)
    {
        ...
        const bool bHasRegularTransforms =
            RegularTransforms != nullptr && !RegularTransforms->IsEmpty();
        const bool bHasHeroTransforms =
            HeroTransforms != nullptr && !HeroTransforms->IsEmpty();
        ...
        if (SpeciesCatalog.IsValidIndex(SpeciesIndex))
        {
            ...
            if (!IsValid(Component->GetTransformProvider()))
            {
                SpeciesTransformProvider =
                    FCubusVegetationAssetResolver::ResolveTransformProvider(SpeciesEntry.SpeciesId);
            }
            ...
            {
                HeroPveActorClass =
                    FCubusVegetationAssetResolver::ResolveHeroPveActorClass(SpeciesEntry);
            }
    ...
    bPublishedVegetationBudgetSaturated =
        ...
        TotalRenderedInstanceCount >= PlantLimit;
    ...
    UE_LOG(
        ...
        Display,
        TEXT(
            "Cubus world vegetation: %d chunks, %d placements, %d rendered instances "
    ...
    if (bEnableRuntimeRandomization && RandomizedPlantCount > 0)
    {
        UE_LOG(
            LogTemp,
            Display,
            TEXT("Cubus per-tree randomization: sampled=%d pruned=%d scaleRange=(%.3f, %.3f)"),
            RandomizedPlantCount,
            RandomPrunedPlantCount,
            ObservedRandomScaleMin,
            ObservedRandomScaleMax
    ...
    )
    {
        UE_LOG(
            ...
            Warning,
            TEXT(
                "Cubus wind diagnostics: no skeletal vegetation instances are active; world batches are static-only. "
    ...
    if (bEnableHeroSkeletalWindMode)
    {
        ...
        if (IsValid(PlayerController) && IsValid(PlayerController->PlayerCameraManager))
        {
            ...
            const double RefreshDistance =
                static_cast<double>(FMath::Max(100.0f, HeroRepresentationRefreshDistance));

            const FIntVector CameraRepresentationCell(
                FMath::FloorToInt(CameraLocation.X / RefreshDistance),
...
ACubusWorldVegetationActor::ResolveVegetationDistancePolicy(const float VoxelSize) const
{
    ...
    Policy.NearResidentRadius =
        (static_cast<float>(FMath::Max(0, CameraChunkHorizontalRadius)) + 0.5f) * ChunkWorldSize;

    const float OverlapDistance = FMath::Min(
        Policy.NearResidentRadius,
        FMath::Max(ChunkWorldSize * 2.0f, Policy.NearResidentRadius * 0.20f)
    );
    ...
    Policy.NearEndCullDistance = FMath::Max(
        1,
        FMath::RoundToInt(
            FMath::Min(
    ...
    Policy.NearStartCullDistance = FMath::Clamp(
        FMath::Min(
            Policy.NearStartCullDistance,
            FMath::RoundToInt(Policy.FarInnerRadius)
        ),
        0,
...
void ACubusWorldVegetationActor::UpdateFarVegetationStreaming(const float DeltaSeconds)
{
    ...
    if (!bLoggedFarStreamingActivation)
    {
        UE_LOG(
            LogTemp,
            Display,
            TEXT("Cubus far streaming entered: enabled=%s blockWorld=%s"),
            bEnableFarVegetation ? TEXT("true") : TEXT("false"),
            IsValid(BlockWorld) ? TEXT("valid") : TEXT("null")
        );
    ...
    const int32 StreamingWorldVoxelX = FMath::FloorToInt(
        (static_cast<double>(StreamingLocation.X) + HalfChunkWorldExtent) /
        static_cast<double>(SafeVoxelSize)
    );

    const int32 StreamingWorldVoxelY = FMath::FloorToInt(
        (static_cast<double>(StreamingLocation.Y) + HalfChunkWorldExtent) /
        static_cast<double>(SafeVoxelSize)
    );

    const FIntPoint CentreCell(
        FMath::FloorToInt(
            static_cast<double>(StreamingWorldVoxelX) /
            static_cast<double>(CellSizeVoxels)
        ),
        FMath::FloorToInt(
            static_cast<double>(StreamingWorldVoxelY) /
            static_cast<double>(CellSizeVoxels)
        )
    );
    ...
    if (CentreCell != LastFarVegetationCentreCell)
    {
        ...
        for (const FIntPoint& Cell : RequiredFarVegetationCells)
        {
            ...
            if (
                ...
                (bHasCachedCell && CompletedPass >= FarVegetationRefinementPass)
            )
            {
                continue;
            }
        ...
        PendingFarVegetationCells.Sort(
            [CentreCell](const FIntPoint& A, const FIntPoint& B)
            {
                const int32 DistanceA =
                    FMath::Abs(A.X - CentreCell.X) + FMath::Abs(A.Y - CentreCell.Y);
                const int32 DistanceB =
                    FMath::Abs(B.X - CentreCell.X) + FMath::Abs(B.Y - CentreCell.Y);
    ...
    )
    {
        ...
        UE_LOG(
            LogTemp,
            Display,
            TEXT("Cubus far vegetation refinement: pass=%d pending=%d"),
            FarVegetationRefinementPass,
    ...
    while (
        FarVegetationBuilds.Num() < SafeConcurrentBuilds &&
    ...
    )
    {
        ...
        if (
            !RequiredFarVegetationCells.Contains(Cell) ||
            (
                FarVegetationCellCache.Contains(Cell) &&
                FarVegetationCellRefinementPasses.FindRef(Cell) >= FarVegetationRefinementPass
            ) ||
            FarVegetationCellsBuilding.Contains(Cell)
        )
        {
        ...
        const FCubusVegetationGenerationSettings GenerationSettings =
            FCubusBlockVegetationGenerator::CaptureGenerationSettings(
                SnapshotChunk->GetGeologyProfile(),
                GenerationSeeds
            );
        ...
        Build.Task = UE::Tasks::Launch(
            ...
            ]()
            {
                ...
                FCubusBlockVegetationGenerator::GenerateFarTreesForRegion(
                    Region,
                    GenerationSeeds,
                    GenerationSettings,
                    DensityField,
...
void ACubusWorldVegetationActor::PublishFarVegetation(
...
)
{
    ...
    if (FarCatalogStaticBatchComponents.IsEmpty())
    {
        ...
        if (!bLoggedMissingFarStaticBatches)
        {
            UE_LOG(
                ...
                Warning,
                TEXT(
                    "Cubus far vegetation cannot publish: no static far-tree batches exist. "
    ...
    const FCubusVegetationDistancePolicy DistancePolicy =
        ResolveVegetationDistancePolicy(SafeVoxelSize);

    const double InnerRadiusSquared =
        FMath::Square(static_cast<double>(DistancePolicy.FarInnerRadius));

    const double OuterRadiusSquared = FMath::Square(
        static_cast<double>(
            FMath::Max(DistancePolicy.FarInnerRadius, FarVegetationEndCullDistance)
        )
    );
    ...
    Cells.Sort(
        [CameraLocation, SafeVoxelSize, CellSizeVoxels, HalfChunkWorldExtent](
            const FIntPoint& A,
            const FIntPoint& B
        )
        {
            const FVector LocationA(
                (static_cast<double>(A.X * CellSizeVoxels) + static_cast<double>(CellSizeVoxels) * 0.5) *
                        SafeVoxelSize -
                    HalfChunkWorldExtent,
                (static_cast<double>(A.Y * CellSizeVoxels) + static_cast<double>(CellSizeVoxels) * 0.5) *
                        SafeVoxelSize -
                    HalfChunkWorldExtent,
                CameraLocation.Z
            );
            ...
            const FVector LocationB(
                (static_cast<double>(B.X * CellSizeVoxels) + static_cast<double>(CellSizeVoxels) * 0.5) *
                        SafeVoxelSize -
                    HalfChunkWorldExtent,
                (static_cast<double>(B.Y * CellSizeVoxels) + static_cast<double>(CellSizeVoxels) * 0.5) *
                        SafeVoxelSize -
                    HalfChunkWorldExtent,
                CameraLocation.Z
            );

            return FVector::DistSquared(LocationA, CameraLocation) <
                   FVector::DistSquared(LocationB, CameraLocation);
        }
    );
    ...
    for (const FIntPoint& Cell : Cells)
    {
        ...
        for (const FCubusVegetationInstance& Instance : *Trees)
        {
            ...
            const FVector BaseWorldLocation(
                (static_cast<double>(Instance.WorldVoxel.X) + 0.5) * SafeVoxelSize - HalfChunkWorldExtent,
            ...
            const FTransform WorldTransform(
                FRotator(0.0f, ResolvedPlacement.Yaw, 0.0f),
                ResolvedPlacement.Location,
                FVector(ResolvedPlacement.Scale)
            );

            TransformsByBatchKey.FindOrAdd(TargetBatchKey).Add(
    ...
    UE_LOG(
        ...
        Display,
        TEXT(
            "Cubus far vegetation: cells=%d trees=%d pending=%d building=%d voxel=%.2fcm pass=%d"
        ),
