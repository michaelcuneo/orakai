														...
														const int32 RiverSeed)
{
	...
	// RockySlopeThreshold is a gradient magnitude, not degrees or a discrete
	// voxel delta. The legacy default of 4.0 was calibrated for a discrete
	// height-range (0-2 voxels) and has been migrated to 1.15 for the new
	// continuous gradient. A gradient of 1.15 corresponds to ~49 degrees.
	// Preserve explicitly authored values that differ from the old default.
...
float FCubusBiomeField::SampleRiverDistance(const float WorldX, const float WorldY, const FCubusBiomeFieldSettings& InSettings)
{
	FCubusBiomeFieldSettings Settings = InSettings;
	...
	const float WarpX	= SampleFbm(RiverX + 7919.0f, RiverY - 3571.0f, Settings.RiverWarpFrequency, 3, 0.5f) * Settings.RiverWarpAmplitude;
	const float WarpY	= SampleFbm(RiverX - 2953.0f, RiverY + 6211.0f, Settings.RiverWarpFrequency, 3, 0.5f) * Settings.RiverWarpAmplitude;
	const float WarpedX = RiverX + WarpX;
	const float WarpedY = RiverY + WarpY;

	const float MainDistance	  = FMath::Abs(SampleFbm(WarpedX, WarpedY, Settings.RiverFrequency, 2, 0.32f));
	const float TributaryDistance = FMath::Abs(SampleFbm(
		WarpedX * 0.78f - WarpedY * 0.63f + 4651.0f, WarpedX * 0.63f + WarpedY * 0.78f - 1307.0f, Settings.RiverFrequency * 1.9f, 2, 0.3f));
	const float Catchment =
		SmoothStep(-0.12f, 0.38f, SampleFbm(WarpedX - 8837.0f, WarpedY + 5279.0f, Settings.RiverFrequency * 0.72f, 2, 0.5f));
	const float GatedTributaryDistance = FMath::Lerp(1.0f, TributaryDistance, Catchment);
...
float FCubusBiomeField::SampleFbm(const float WorldX, const float WorldY, const float Frequency, const int32 Octaves, const float Gain)
{
	...
	{
		Sum += SampleNoise(WorldX + static_cast<float>(Octave) * 1747.0f, WorldY - static_cast<float>(Octave) * 2081.0f, CurrentFrequency) *
			   Weight;
		TotalWeight += Weight;
