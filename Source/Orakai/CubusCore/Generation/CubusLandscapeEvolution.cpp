...
FProvinceParameters FGenerator::GetDefaultProvinceParameters(const EProvinceType Province)
{
	...
	case EProvinceType::VolcanicProvince:
		...
		break;
	case EProvinceType::OceanicBasin:
		Result = {0.45f, 0.35f, 0.80f, 0.65f};
		break;
...
bool FGenerator::GenerateSkeleton(const FSettings& Settings, FGlobalDem& OutDem, FGenerationStats* OutStats, FString* OutError)
{
	if (Settings.Resolution < 17 || Settings.PlateCount < 2 || Settings.PlateCount > 255 || Settings.WorldSizeMeters <= 0.0 ||
		Settings.CrustBlendScaleM <= 0.0f)
	{
		if (OutError)
		{
	...
	for (int32 I = 0; I < Settings.PlateCount; ++I)
	{
		...
		Plate.bOceanic			 = I == 1 || (I > 1 && Random.FRand() < 0.34f);
		Plate.BaseElevationM	 = Plate.bOceanic ? Random.FRandRange(-2200.0f, -450.0f) : Random.FRandRange(120.0f, 650.0f);
		Plate.CrustStrength		 = Random.FRandRange(0.65f, 1.4f);
		Plate.Age01				 = Random.FRand();
		Plate.bVolcanic			 = Random.FRand() < (Plate.bOceanic ? 0.18f : 0.10f);
		const float ProvinceRoll = Random.FRand();
		Plate.InteriorProvince	 = Plate.bOceanic		  ? EProvinceType::OceanicBasin
								   : ProvinceRoll < 0.30f ? EProvinceType::StablePlain
								   ...
								   : Plate.bVolcanic	  ? EProvinceType::VolcanicProvince
	...
	const double CrustSigma01		= FMath::Max(1.0, static_cast<double>(Settings.CrustBlendScaleM)) / Settings.WorldSizeMeters;
	const double InvTwoCrustSigmaSq = 1.0 / (2.0 * CrustSigma01 * CrustSigma01);

	ParallelFor(CellCount,
				[&OutDem, &Settings, R, Half, InvIntervals, InvTwoCrustSigmaSq](const int32 Cell)
				{
					...
					double PlateDistancesSquared[256];
					...
					const float Warp  = ValueNoise2D(WorldMeters / 35000.0, SymmetricBoundarySeed(Settings, Closest, Second));
					BoundaryDistanceM = FMath::Max(0.0f, BoundaryDistanceM + Warp * Settings.BoundaryWarpAmplitudeM);

					auto SamplePairUplift = [&OutDem, &Settings, &WorldMeters](const int32 PlateAId, const double PlateAD2,
																			   const int32 PlateBId, const double PlateBD2)
					{
						const FPlate&		PlateA			 = OutDem.Plates[PlateAId];
						const FPlate&		PlateB			 = OutDem.Plates[PlateBId];
						const FVector2D		Axis			 = (PlateB.Center01 - PlateA.Center01) * Settings.WorldSizeMeters;
						float				PairNormalSpeed	 = 0.0f;
						float				PairTangentSpeed = 0.0f;
						const EBoundaryType PairType		 = ClassifyBoundary(PlateA, PlateB, Axis, PairNormalSpeed, PairTangentSpeed);
						const double		PlateADistanceM	 = FMath::Sqrt(PlateAD2) * Settings.WorldSizeMeters;
						const double		PlateBDistanceM	 = FMath::Sqrt(PlateBD2) * Settings.WorldSizeMeters;
						float				PairDistanceM	 = static_cast<float>(0.5 * FMath::Abs(PlateBDistanceM - PlateADistanceM));
						const float PairWarp = ValueNoise2D(WorldMeters / 35000.0, SymmetricBoundarySeed(Settings, PlateAId, PlateBId));
						PairDistanceM		 = FMath::Max(0.0f, PairDistanceM + PairWarp * Settings.BoundaryWarpAmplitudeM);
						float PairUplift	 = BoundaryContribution(PairType, PairDistanceM, Settings);
						if (PairType == EBoundaryType::Convergent)
						{
							PairUplift *= FMath::Clamp((FMath::Abs(PairNormalSpeed) + 0.05f) * 3.0f, 0.45f, 1.5f);
						}
						return PairUplift;
					};

					constexpr int32 RelevantPlateCount				= 4;
					int32			RelevantIds[RelevantPlateCount] = {INDEX_NONE, INDEX_NONE, INDEX_NONE, INDEX_NONE};
					double			RelevantD2[RelevantPlateCount]	= {TNumericLimits<double>::Max(), TNumericLimits<double>::Max(),
																	   TNumericLimits<double>::Max(), TNumericLimits<double>::Max()};
					for (int32 CandidateId = 0; CandidateId < OutDem.Plates.Num(); ++CandidateId)
					{
						const double CandidateD2 = PlateDistancesSquared[CandidateId];
						for (int32 Slot = 0; Slot < RelevantPlateCount; ++Slot)
						{
							if (CandidateD2 < RelevantD2[Slot])
							{
								for (int32 Move = RelevantPlateCount - 1; Move > Slot; --Move)
								{
									RelevantD2[Move]  = RelevantD2[Move - 1];
									RelevantIds[Move] = RelevantIds[Move - 1];
								}
								RelevantD2[Slot]  = CandidateD2;
								RelevantIds[Slot] = CandidateId;
								break;
							}
						}
					}

					const double ClosestDistanceM = FMath::Sqrt(ClosestD2) * Settings.WorldSizeMeters;
					float		 UpliftWeighted	  = 0.0f;
					float		 UpliftWeightSum  = 0.0f;
					for (int32 A = 0; A < RelevantPlateCount - 1 && RelevantIds[A] != INDEX_NONE; ++A)
					{
						for (int32 B = A + 1; B < RelevantPlateCount && RelevantIds[B] != INDEX_NONE; ++B)
						{
							const double PairMeanDistanceM =
								0.5 * (FMath::Sqrt(RelevantD2[A]) + FMath::Sqrt(RelevantD2[B])) * Settings.WorldSizeMeters;
							const float PairWeight = FMath::Exp(-static_cast<float>(FMath::Max(0.0, PairMeanDistanceM - ClosestDistanceM)) /
																FMath::Max(1000.0f, Settings.BoundaryWidthM));
							UpliftWeighted += SamplePairUplift(RelevantIds[A], RelevantD2[A], RelevantIds[B], RelevantD2[B]) * PairWeight;
							UpliftWeightSum += PairWeight;
						}
					}
					const float Uplift = UpliftWeightSum > SMALL_NUMBER ? UpliftWeighted / UpliftWeightSum : 0.0f;
					...
					if (BoundaryDistanceM < Settings.BoundaryWidthM * 0.9f)
					{
						Province = Type == EBoundaryType::Convergent  ? EProvinceType::FoldMountainBelt
								   : Type == EBoundaryType::Divergent ? EProvinceType::RiftValley
																	  : Province;
					}
					...
					const float ContinuousBaseElevation =
						CrustWeightSum > SMALL_NUMBER ? static_cast<float>(CrustElevationWeighted / CrustWeightSum) : 250.0f;
					...
					if (Elevation <= Settings.OceanLevelM && Elevation >= Settings.OceanLevelM - 500.0f)
					{
	...
	if (OutStats)
	{
		...
		MeasureQuality(OutDem, *OutStats);
	}
	return true;
}

void FGenerator::MeasureQuality(const FGlobalDem& Dem, FGenerationStats& OutStats)
{
	if (!Dem.IsValid())
	{
		return;
	...
	int64		 LandCells			   = 0;
	int64		 SteepLandCells		   = 0;
	double		 TotalSlopeDegrees	   = 0.0;
	float		 MaximumNeighbourStepM = 0.0f;
	const int32	 R					   = Dem.Resolution;
	const double CellSize			   = FMath::Max(1.0, Dem.CellSizeMeters);
	for (int32 Y = 1; Y < R - 1; ++Y)
	{
		for (int32 X = 1; X < R - 1; ++X)
		{
			const int32 Cell	  = Dem.Index(X, Y);
			MaximumNeighbourStepM = FMath::Max(MaximumNeighbourStepM, FMath::Abs(Dem.ElevationM[Cell] - Dem.ElevationM[Cell + 1]));
			MaximumNeighbourStepM = FMath::Max(MaximumNeighbourStepM, FMath::Abs(Dem.ElevationM[Cell] - Dem.ElevationM[Cell + R]));
			if (Dem.ElevationM[Cell] <= Dem.OceanLevelM)
			{
				continue;
			}

			const double GradientX	  = (Dem.ElevationM[Cell + 1] - Dem.ElevationM[Cell - 1]) / (2.0 * CellSize);
			const double GradientY	  = (Dem.ElevationM[Cell + R] - Dem.ElevationM[Cell - R]) / (2.0 * CellSize);
			const double SlopeDegrees = FMath::RadiansToDegrees(FMath::Atan(FMath::Sqrt(GradientX * GradientX + GradientY * GradientY)));
			TotalSlopeDegrees += SlopeDegrees;
			++LandCells;
			SteepLandCells += SlopeDegrees >= 35.0 ? 1 : 0;
		}
	}

	const int64 InteriorCells = static_cast<int64>(FMath::Max(0, R - 2)) * static_cast<int64>(FMath::Max(0, R - 2));
	OutStats.LandFraction =
		InteriorCells > 0 ? static_cast<float>(static_cast<double>(LandCells) / static_cast<double>(InteriorCells)) : 0.0f;
	OutStats.MeanLandSlopeDegrees = LandCells > 0 ? static_cast<float>(TotalSlopeDegrees / static_cast<double>(LandCells)) : 0.0f;
	OutStats.SteepLandFraction =
		LandCells > 0 ? static_cast<float>(static_cast<double>(SteepLandCells) / static_cast<double>(LandCells)) : 0.0f;
	OutStats.MaximumNeighbourStepM = MaximumNeighbourStepM;
}
...
bool FGenerator::SolveHydrology(const FSettings& Settings, FGlobalDem& InOutDem, FGenerationStats* OutStats, FString* OutError)
{
	...
	if (InOutDem.FlowOrder.Num() != N)
	{
		...
		{
			*OutError =
				FString::Printf(TEXT("Hydrology receiver graph is not acyclic: ordered %d of %d cells."), InOutDem.FlowOrder.Num(), N);
		}
		return false;
	}
	...
	for (int32 OrderIndex = InOutDem.FlowOrder.Num() - 1; OrderIndex >= 0; --OrderIndex)
	{
		...
		InOutDem.DistanceToOutletKm[Cell] =
			InOutDem.DistanceToOutletKm[Receiver] + static_cast<float>(ReceiverDistanceMeters(InOutDem, Cell, Receiver) / 1000.0);
	}
