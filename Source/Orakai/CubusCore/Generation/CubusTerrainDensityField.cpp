...
{
	return static_cast<float>(HashCoordinates(X, Y, Seed) & 0x00ffffffu) / static_cast<float>(0x00ffffffu);
}
...
FVector CaveNode(const int32 CellX, const int32 CellY, const FCubusTerrainDensitySettings& Settings, const uint32 Seed)
{
	...
	const float NodeZ = FMath::Lerp(MinimumZ + VerticalMargin, MaximumZ - VerticalMargin, Hash01(CellX, CellY, Seed ^ 0xAD90777Du));

	return FVector((static_cast<float>(CellX) + 0.5f) * CellSize + JitterX, (static_cast<float>(CellY) + 0.5f) * CellSize + JitterY, NodeZ);
}
...
float DistanceToSegment(const FVector& Point, const FVector& A, const FVector& B)
{
	...
	const double Alpha = FMath::Clamp(FVector::DotProduct(Point - A, Segment) / LengthSquared, 0.0, 1.0);
...
FCubusTerrainDensityField::FCubusTerrainDensityField(const FCubusTerrainDensitySettings& InSettings) : Settings(InSettings)
{
	...
	HydrologySettings.RiverSeed		   = Settings.RiverSeed != 0 ? Settings.RiverSeed : Settings.BiomeSettings.HydrologySettings.RiverSeed;
	...
	HydrologySettings.ValleyHalfWidth  = FMath::Max(HydrologySettings.ChannelHalfWidth + 8.0f, Settings.RiverValleyWidth * 160.0f);
...
{
	return SampleContinuous(FVector(static_cast<double>(GlobalSampleCoordinate.X), static_cast<double>(GlobalSampleCoordinate.Y),
...
FCubusDensitySample FCubusTerrainDensityField::SampleContinuous(const FVector& GlobalSampleCoordinate) const
{
	...
	if (!Settings.bPreserveAuthoredHeightfield && Settings.bUseHeightTerrain && FMath::Abs(MacroTerrainDensity) < 2.5f)
	{
		...
		const float MesoscopicNoise =
			SampleNoise2D(WorldX + 42017.0f, WorldY - 18311.0f, FMath::Max(0.30f, Settings.DetailFrequency * 3.8f));
		const float FineNoise		= SampleNoise2D(WorldX * 0.79f - WorldY * 0.61f - 11719.0f, WorldX * 0.61f + WorldY * 0.79f + 23801.0f,
													FMath::Max(0.68f, Settings.DetailFrequency * 8.5f));
		const float FineWarp		= SampleNoise2D(WorldX - 7193.0f, WorldY + 3119.0f, FMath::Max(0.16f, Settings.DetailFrequency * 2.0f));
		const float WarpedFineNoise = SampleNoise2D(WorldX + FineWarp * 0.42f + 13007.0f, WorldY - FineWarp * 0.42f - 9113.0f,
													FMath::Max(1.05f, Settings.DetailFrequency * 13.0f));

		const float FineAmplitude = FMath::Lerp(0.08f, 0.34f, Roughness) * ValleyCalm * MountainTexture;
		FineSurfaceDisplacement =
			FMath::Clamp((MesoscopicNoise * 0.54f + FineNoise * 0.31f + WarpedFineNoise * 0.15f) * FineAmplitude, -0.34f, 0.34f);
	}
	...
	{
		Result.MaterialId =
			GetCachedSurfaceMaterialData(static_cast<float>(GlobalSampleCoordinate.X), static_cast<float>(GlobalSampleCoordinate.Y))
				.SurfaceMaterialId;
	}
...
const FCubusTerrainDensityField::FSurfaceData& FCubusTerrainDensityField::GetCachedSurfaceData(const float WorldSampleX,
																							   const float WorldSampleY) const
{
...
const FCubusTerrainDensityField::FColumnData& FCubusTerrainDensityField::GetColumnData(const float WorldSampleX,
																					   const float WorldSampleY) const
{
	const FIntPoint Key = MakeCoordinateCacheKey(WorldSampleX, WorldSampleY);
	...
	const float HardnessSignal = SampleNoise2D(TerrainX + 19031.0f, TerrainY - 12763.0f, Settings.GeologyHardnessFrequency);
	Column.RockHardness		   = FMath::Clamp(0.5f + HardnessSignal * 0.5f, 0.0f, 1.0f);

	const float FractureLines	 = SampleRidgedNoise(TerrainX - 7127.0f, TerrainY + 15551.0f, Settings.GeologyFractureFrequency);
	const float FractureProvince = FMath::Clamp(
		0.5f + SampleNoise2D(TerrainX + 3721.0f, TerrainY + 9281.0f, Settings.GeologyHardnessFrequency * 0.72f) * 0.5f, 0.0f, 1.0f);
	Column.Fracture	  = FMath::Clamp(FractureLines * FractureProvince, 0.0f, 1.0f);
	Column.StrataTilt = SampleNoise2D(TerrainX - 1091.0f, TerrainY - 23981.0f, Settings.GeologyFoldFrequency);

	const float CliffExposure	 = SmoothStep(Settings.GeologyCliffSlopeStart, Settings.GeologyCliffSlopeFull, Column.Slope);
	const float LandformExposure = FMath::Clamp(
		Column.FormSample.MountainCore * 0.82f + Column.FormSample.FoothillWeight * 0.46f + Column.FormSample.Ridge * 0.32f, 0.0f, 1.0f);
	const float DrainageProtection = 1.0f - FMath::Clamp(Column.FormSample.Drainage * Column.FormSample.Drainage, 0.0f, 1.0f);
...
const FCubusTerrainDensityField::FSurfaceMaterialData&
FCubusTerrainDensityField::GetCachedSurfaceMaterialData(const float WorldSampleX, const float WorldSampleY) const
{
	/*
	...
	MaterialData.SubsurfaceMaterialId =
		Settings.BiomeSettings.bEnabled ? Settings.BiomeSubsurfaceMaterialId : Settings.SubsurfaceMaterialId;
	MaterialData.BedrockMaterialId = Settings.BiomeSettings.bEnabled ? Settings.BiomeRockMaterialId : Settings.RockMaterialId;
	if (Settings.bUseFastMaterialClassification)
	{
		MaterialData.SurfaceMaterialId = Column.Slope >= Settings.RockSlopeThreshold ? Settings.RockMaterialId : Settings.SurfaceMaterialId;
	...
	MaterialData.BiomeSample = FCubusBiomeField::Sample(WorldX, WorldY, Column.SurfaceVoxelHeight, Column.Slope, Settings.BiomeSettings,
														BiomeTerrainContext, &Climate);

	const FCubusLandmarkSample LandmarkSample			= FCubusLandmarkField::Sample(TerrainX, TerrainY, Settings.LandmarkSettings);
	const bool				   bIsLandmark				= LandmarkSample.IsInside();
	const bool				   bHasAuthoredSurfaceRules = Settings.MaterialSelectionRules.ContainsByPredicate(
		[](const FCubusCompiledTerrainMaterialRule& Rule) { return Rule.Selection.Role == ECubusTerrainMaterialRole::Surface; });
	if (bIsLandmark)
	{
	...
	{
		MaterialData.SurfaceMaterialId =
			Column.Slope >= Settings.RockSlopeThreshold
		...
		const float StructuralSnowFloor = Settings.BaseHeight + FMath::Max(64.0f, Settings.RidgeAmplitude * 4.0f);
		const float ConfiguredSnowLine	= Settings.BiomeSettings.bEnabled ? Settings.BiomeSnowMinimumHeight : Settings.SnowMinimumHeight;
		const float SnowLine			= FMath::Max(ConfiguredSnowLine, StructuralSnowFloor);
		const float SlopeRetention =
			1.0f - SmoothStep(Settings.RockSlopeThreshold * 0.95f,
							  FMath::Max(Settings.RockSlopeThreshold * 1.70f, Settings.RockSlopeThreshold + 0.01f), Column.Slope);

		/*
		 * Snow is an accumulation field, not an elevation contour. Broad noise
		 * shifts the local snow line into coherent drifts while climate, aspect,
		 * shelter and slope decide whether snow is retained at that location.
		 */
		const float SnowBandWidth = CubusWorldScale::MetersToVoxels(300.0f, Settings.VoxelSizeCm);
		const float BroadDrift	  = SampleNoise2D(TerrainX + 23117.0f, TerrainY - 17011.0f, 0.0022f);
		const float LocalDrift	  = SampleNoise2D(TerrainX - 7127.0f, TerrainY + 11939.0f, 0.0095f);
		const float WindScour	  = MaterialData.BiomeSample.WindExposure * 0.72f + MaterialData.BiomeSample.SolarExposure * 0.34f -
									MaterialData.BiomeSample.WindShelter * 0.24f;
		const float LocalSnowLine =
			SnowLine + BroadDrift * SnowBandWidth * 0.58f + LocalDrift * SnowBandWidth * 0.16f + WindScour * SnowBandWidth * 0.38f;

		const float AltitudeAccumulation =
			SmoothStep(LocalSnowLine - SnowBandWidth * 0.62f, LocalSnowLine + SnowBandWidth * 0.72f, Column.SurfaceVoxelHeight);
		const float ColdRetention  = FMath::Lerp(0.58f, 1.12f, 1.0f - MaterialData.BiomeSample.Temperature);
		const float MoistureSupply = FMath::Lerp(
			0.68f, 1.10f,
			FMath::Clamp(MaterialData.BiomeSample.Moisture * 0.55f + MaterialData.BiomeSample.OrographicLift * 0.45f, 0.0f, 1.0f));
		const float LandformRetention = FMath::Lerp(
			0.82f, 1.14f,
			FMath::Clamp(MaterialData.BiomeSample.RidgeInfluence * 0.35f + MaterialData.BiomeSample.WindShelter * 0.65f, 0.0f, 1.0f));
		const float NivalRetention =
			FMath::Clamp(MaterialData.BiomeSample.NivalInfluence * 0.82f + MaterialData.BiomeSample.AlpineInfluence * 0.18f, 0.0f, 1.0f);
		const float FineDrift = SampleNoise2D(TerrainX + 4721.0f, TerrainY - 8923.0f, 0.027f);
		const float DriftPattern =
			FMath::Clamp(0.50f + BroadDrift * 0.26f + LocalDrift * 0.22f + FineDrift * 0.16f -
							 MaterialData.BiomeSample.WindExposure * 0.24f + MaterialData.BiomeSample.WindShelter * 0.16f,
						 0.0f, 1.0f);
		const float PatchRetention	  = SmoothStep(0.28f, 0.72f, DriftPattern);
		const float BaseAccumulation  = FMath::Max(NivalRetention * SlopeRetention, AltitudeAccumulation * SlopeRetention * ColdRetention *
																						MoistureSupply * LandformRetention);
		const float SnowAccumulation  = FMath::Clamp(BaseAccumulation * FMath::Lerp(0.16f, 1.08f, PatchRetention), 0.0f, 1.0f);
		const float PatchThreshold	  = FMath::Lerp(0.48f, 0.34f, PatchRetention);
		MaterialData.SnowAccumulation = SnowAccumulation;

		if (!bHasAuthoredSurfaceRules && SnowAccumulation >= PatchThreshold)
		{
			MaterialData.SurfaceMaterialId = Settings.BiomeSettings.bEnabled ? Settings.BiomeSnowMaterialId : Settings.SnowMaterialId;
		}
	}

	if (!Settings.MaterialSelectionRules.IsEmpty())
	{
		if (!bIsLandmark)
		{
			MaterialData.SurfaceMaterialId = SelectMaterialRule(ECubusTerrainMaterialRole::Surface, WorldX, WorldY, MaterialData, Column,
																MaterialData.SurfaceMaterialId);
		}
		MaterialData.SubsurfaceMaterialId = SelectMaterialRule(ECubusTerrainMaterialRole::Subsurface, WorldX, WorldY, MaterialData, Column,
															   MaterialData.SubsurfaceMaterialId);
		MaterialData.BedrockMaterialId =
			SelectMaterialRule(ECubusTerrainMaterialRole::Bedrock, WorldX, WorldY, MaterialData, Column, MaterialData.BedrockMaterialId);
	}
...
int32 FCubusTerrainDensityField::SelectMaterialRule(const ECubusTerrainMaterialRole Role, const float WorldX, const float WorldY,
													const FSurfaceMaterialData& MaterialData, const FColumnData& Column,
													const int32 FallbackMaterialId) const
{
	const FCubusBiomeSample& Biome		   = MaterialData.BiomeSample;
	auto					 ResolveSignal = [&Biome, &MaterialData, &Column](const ECubusTerrainMaterialSignal Signal)
	{
		switch (Signal)
		{
		case ECubusTerrainMaterialSignal::SurfaceWorldZ:
			return Column.SurfaceVoxelHeight;
		case ECubusTerrainMaterialSignal::ElevationNormalized:
			return Biome.ElevationNormalized;
		case ECubusTerrainMaterialSignal::Slope:
			return Column.Slope;
		case ECubusTerrainMaterialSignal::Temperature:
			return Biome.Temperature;
		case ECubusTerrainMaterialSignal::Moisture:
			return Biome.Moisture;
		case ECubusTerrainMaterialSignal::SoilMoisture:
			return Biome.SoilMoisture;
		case ECubusTerrainMaterialSignal::SoilSaturation:
			return Biome.SoilSaturation;
		case ECubusTerrainMaterialSignal::Drainage:
			return Biome.Drainage;
		case ECubusTerrainMaterialSignal::RiverInfluence:
			return Biome.RiverInfluence;
		case ECubusTerrainMaterialSignal::RockExposure:
			return Biome.RockExposure;
		case ECubusTerrainMaterialSignal::SoilDepth:
			return Biome.SoilDepth;
		case ECubusTerrainMaterialSignal::OrganicMatter:
			return Biome.OrganicMatter;
		case ECubusTerrainMaterialSignal::Fertility:
			return Biome.Fertility;
		case ECubusTerrainMaterialSignal::SubstrateHardness:
			return Biome.SubstrateHardness;
		case ECubusTerrainMaterialSignal::FractureDensity:
			return Biome.FractureDensity;
		case ECubusTerrainMaterialSignal::WindExposure:
			return Biome.WindExposure;
		case ECubusTerrainMaterialSignal::WindShelter:
			return Biome.WindShelter;
		case ECubusTerrainMaterialSignal::SolarExposure:
			return Biome.SolarExposure;
		case ECubusTerrainMaterialSignal::OrographicLift:
			return Biome.OrographicLift;
		case ECubusTerrainMaterialSignal::RainShadow:
			return Biome.RainShadow;
		case ECubusTerrainMaterialSignal::FloodplainInfluence:
			return Biome.FloodplainInfluence;
		case ECubusTerrainMaterialSignal::ValleyFloorInfluence:
			return Biome.ValleyFloorInfluence;
		case ECubusTerrainMaterialSignal::RidgeInfluence:
			return Biome.RidgeInfluence;
		case ECubusTerrainMaterialSignal::AlpineInfluence:
			return Biome.AlpineInfluence;
		case ECubusTerrainMaterialSignal::NivalInfluence:
			return Biome.NivalInfluence;
		case ECubusTerrainMaterialSignal::SnowAccumulation:
			return MaterialData.SnowAccumulation;
		default:
			return 0.0f;
		}
	};

	auto Suitability = [](const float Value, const FCubusTerrainMaterialSignalRule& Rule)
	{
		if (Value >= Rule.Minimum && Value <= Rule.Maximum)
		{
			return 1.0f;
		}
		const float Width = FMath::Max(0.0f, Rule.BlendWidth);
		if (Width <= UE_SMALL_NUMBER)
		{
			return 0.0f;
		}
		if (Value < Rule.Minimum)
		{
			return FMath::SmoothStep(Rule.Minimum - Width, Rule.Minimum, Value);
		}
		return 1.0f - FMath::SmoothStep(Rule.Maximum, Rule.Maximum + Width, Value);
	};

	int32 WinningMaterialId = FMath::Max(1, FallbackMaterialId);
	float WinningScore		= -MAX_flt;
	int32 WinningPriority	= MIN_int32;
	for (const FCubusCompiledTerrainMaterialRule& Rule : Settings.MaterialSelectionRules)
	{
		if (Rule.Selection.Role != Role)
		{
			continue;
		}

		float CommunityMatch = Rule.Selection.CommunityNames.IsEmpty() ? 1.0f : 0.0f;
		if (!Rule.Selection.CommunityNames.IsEmpty())
		{
			for (int32 Index = 0; Index < Biome.CommunityBlendCount; ++Index)
			{
				if (Rule.Selection.CommunityNames.Contains(Biome.CommunityBlend[Index].Name))
				{
					CommunityMatch += Biome.CommunityBlend[Index].Weight;
				}
			}
		}

		float Score		= Rule.Selection.BaseScore + Rule.Selection.CommunityScoreWeight * CommunityMatch;
		bool  bRejected = false;
		for (const FCubusTerrainMaterialSignalRule& SignalRule : Rule.Selection.Signals)
		{
			const float SignalSuitability = Suitability(ResolveSignal(SignalRule.Signal), SignalRule);
			if (SignalRule.bRequired && SignalSuitability <= UE_SMALL_NUMBER)
			{
				bRejected = true;
				break;
			}
			Score += SignalRule.ScoreWeight * SignalSuitability;
		}
		if (bRejected)
		{
			continue;
		...
		if (Rule.Selection.VariationFrequency > 0.0f && Rule.Selection.VariationScoreAmplitude > 0.0f)
		{
			const float SaltX = static_cast<float>(Rule.Selection.VariationSalt ^ Rule.MaterialId) * 17.0f;
			const float SaltY = static_cast<float>(Rule.Selection.VariationSalt + Rule.MaterialId * 31) * -11.0f;
			Score +=
				SampleNoise2D(WorldX + SaltX, WorldY + SaltY, Rule.Selection.VariationFrequency) * Rule.Selection.VariationScoreAmplitude;
		}

		if (Score < Rule.Selection.MinimumWinningScore)
		{
			continue;
		}

		const bool bBetterScore = Score > WinningScore + 1.0e-6f;
		const bool bTiedBetterPriority =
			FMath::IsNearlyEqual(Score, WinningScore, 1.0e-6f) && Rule.Selection.TieBreakPriority > WinningPriority;
		const bool bTiedLowerId = FMath::IsNearlyEqual(Score, WinningScore, 1.0e-6f) &&
								  Rule.Selection.TieBreakPriority == WinningPriority && Rule.MaterialId < WinningMaterialId;
		if (bBetterScore || bTiedBetterPriority || bTiedLowerId)
		{
			WinningScore	  = Score;
			WinningPriority	  = Rule.Selection.TieBreakPriority;
			WinningMaterialId = Rule.MaterialId;
		}
	}

	return WinningMaterialId;
}
...
{
	const FIntPoint Key(FMath::RoundToInt(WorldX * BiomeMacroHeightCacheScale), FMath::RoundToInt(WorldY * BiomeMacroHeightCacheScale));
	 ...
	 */
	const float Height =
		Settings.bUseHeightTerrain ? FCubusTerrainForm::Sample(TerrainX, TerrainY, TerrainFormSettings).Height : Settings.FlatSurfaceWorldZ;
	BiomeMacroHeightCache.Add(Key, Height);
	return Height;
}

const FCubusBiomeTopographicClimateContext&
FCubusTerrainDensityField::GetCachedBiomeTopographicClimateCell(const FIntPoint& CellCoordinate) const
{
	...
	for (int32 Index = 0; Index < UE_ARRAY_COUNT(Distances); ++Index)
	{
		...
		const float UpwindHeight = GetCachedBiomeMacroHeight(WorldX - Wind.X * Distance, WorldY - Wind.Y * Distance);
		const float RelativeLift = FMath::Clamp((LocalHeight - UpwindHeight) / FMath::Max(8.0f, Distance * 0.34f), 0.0f, 1.0f);
		Lift += RelativeLift * Weight;
		RainBarrier = FMath::Max(RainBarrier, FMath::Clamp((UpwindHeight - LocalHeight) / FMath::Max(8.0f, Distance * 0.28f), 0.0f, 1.0f) *
												  FMath::Lerp(1.0f, 0.72f, static_cast<float>(Index) / 2.0f));

		const float SolarHeight = GetCachedBiomeMacroHeight(WorldX + Solar.X * Distance, WorldY + Solar.Y * Distance);
		SolarBarrier = FMath::Max(SolarBarrier, FMath::Clamp((SolarHeight - LocalHeight) / FMath::Max(8.0f, Distance * 0.24f), 0.0f, 1.0f) *
													FMath::Lerp(1.0f, 0.74f, static_cast<float>(Index) / 2.0f));
	}

	static const FVector2D SkyDirections[] = {FVector2D(1.0f, 0.0f),
											  FVector2D(-1.0f, 0.0f),
											  FVector2D(0.0f, 1.0f),
											  FVector2D(0.0f, -1.0f),
	...
	{
		const float HorizonHeight = GetCachedBiomeMacroHeight(WorldX + Direction.X * SkyDistance, WorldY + Direction.Y * SkyDistance);
		HorizonClosure += FMath::Clamp((HorizonHeight - LocalHeight) / (SkyDistance * 0.32f), 0.0f, 1.0f);
	}
...
FCubusBiomeTopographicClimateContext FCubusTerrainDensityField::GetInterpolatedBiomeTopographicClimate(const float WorldX,
																									   const float WorldY) const
{
	...
	auto LerpContext = [](const FCubusBiomeTopographicClimateContext& A, const FCubusBiomeTopographicClimateContext& B, const float Alpha)
	{
		FCubusBiomeTopographicClimateContext Result;
...
FCubusTerrainDensityField::FTerrainRegionWeights FCubusTerrainDensityField::SampleTerrainRegions(const float WorldX,
																								 const float WorldY) const
{
	const float RegionNoise = SampleNoise2D(WorldX + 10427.0f, WorldY - 8633.0f, Settings.RegionFrequency);
	const float PlainsExit =
		SmoothStep(Settings.PlainsThreshold - Settings.PlainsBlend, Settings.PlainsThreshold + Settings.PlainsBlend, RegionNoise);
	const float MountainEntry =
		SmoothStep(Settings.MountainThreshold - Settings.MountainBlend, Settings.MountainThreshold + Settings.MountainBlend, RegionNoise);
...
float FCubusTerrainDensityField::SampleRidgedNoise3D(const float WorldX, const float WorldY, const float WorldZ,
...
float FCubusTerrainDensityField::ApplyRiverLowering(const float SurfaceHeight, const float WorldX, const float WorldY) const
{
	...
	return ApplyRiverLowering(SurfaceHeight, FCubusHydrologyField::Sample(WorldX, WorldY, HydrologySettings));
}

float FCubusTerrainDensityField::ApplyRiverLowering(const float SurfaceHeight, const FCubusHydrologySample& Hydrology) const
{
	...
	const float ChannelHalfWidth =
		FMath::Lerp(HydrologySettings.ChannelHalfWidth * 0.75f, HydrologySettings.ChannelHalfWidth * 1.35f, Hydrology.ChannelStrength);
	const float ValleyHalfWidth = FMath::Lerp(ChannelHalfWidth * 2.5f, HydrologySettings.ValleyHalfWidth, Hydrology.ChannelStrength);
														 ...
														 const float BaseTerrainDensity) const
{
	 ...
	 */
	const float StructuralCarrier = FMath::Clamp(Column.FormSample.Escarpment * 0.62f + Column.FormSample.MountainCore * 0.22f +
													 Column.FormSample.MassifWeight * 0.18f + Column.FormSample.Ridge * 0.12f,
												 0.0f, 1.0f);
	const float ValleyProtection  = 1.0f - FMath::Clamp(Column.FormSample.ValleyCarve * 0.86f, 0.0f, 0.92f);
	const float GeologyCarrier =
		FMath::Clamp(Column.RockExposure * FMath::Lerp(0.34f, 1.0f, StructuralCarrier) * ValleyProtection, 0.0f, 1.0f);
	...
	const float StrataFrequency = FMath::Clamp(Settings.GeologyStrataFrequency, 0.008f, 0.16f);
	const float FoldedDip		= FMath::Clamp(Settings.GeologyStrataDip + Column.StrataTilt * 0.012f, -0.035f, 0.035f);
	const float StrataPhase =
		WorldZ * StrataFrequency + Across * FoldedDip + SampleNoise2D(Along + 9113.0f, Across - 4327.0f, StrataFrequency * 0.16f) * 0.20f;
	...
	const float RockMass	   = SampleNoise3D(WorldX - 6211.0f, WorldY + 4177.0f, WorldZ - 1987.0f, RockMassFrequency);
	const float RockWarp	   = SampleNoise3D(WorldX + 1709.0f, WorldY - 3187.0f, WorldZ + 733.0f, RockWarpFrequency);
	const float FractureVolume = SampleRidgedNoise3D(WorldX + 12011.0f, WorldY - 4919.0f, WorldZ + 2791.0f, FractureFrequency);
	const float FractureCut	   = SmoothStep(0.80f, 0.97f, FractureVolume) * Column.Fracture;
	 ...
	 */
	const float OverhangSignal	   = SampleNoise3D(Across + 2381.0f, Along + 7151.0f, WorldZ - 3319.0f, 0.11f);
	const float OverhangPermission = SmoothStep(0.42f, 0.88f, StructuralCarrier) * Column.RockHardness;

	const float RawDeformation = StrataWave * 0.22f * FMath::Lerp(0.62f, 1.0f, Column.RockHardness) + RockMass * 0.24f + RockWarp * 0.12f +
								 OverhangSignal * 0.30f * OverhangPermission - FractureCut * 0.20f;

	const float MaximumDisplacement	   = FMath::Lerp(0.16f, 0.72f, GeologyCarrier) * SurfaceBandMask;
	const float GeologicalDisplacement = FMath::Clamp(RawDeformation, -MaximumDisplacement, MaximumDisplacement);

	return BaseTerrainDensity + GeologicalDisplacement;
}

float FCubusTerrainDensityField::SampleCaveDensity(const FVector& GlobalSampleCoordinate, const float SurfaceVoxelHeight,
												   const float SurfaceSlope) const
{
	const float WorldZ				  = static_cast<float>(GlobalSampleCoordinate.Z);
	...
	const FVector Query(GlobalSampleCoordinate.X + static_cast<double>(Settings.CaveOffsetX),
	...
	for (int32 CellY = CentreCellY - 1; CellY <= CentreCellY + 1; ++CellY)
	{
		...
		{
			const float Activity = CubusTerrainDensityField::Hash01(CellX, CellY, Seed ^ 0xB5297A4Du);
			...
			const float	  DirectionChoice = CubusTerrainDensityField::Hash01(CellX, CellY, Seed ^ 0x68E31DA4u);
			...
			const float RadiusVariation = FMath::Lerp(0.72f, 1.28f, CubusTerrainDensityField::Hash01(CellX, CellY, Seed ^ 0x1B56C4E9u));
			...
			{
				const float ChamberVariation =
					FMath::Lerp(0.75f, 1.25f, CubusTerrainDensityField::Hash01(CellX, CellY, Seed ^ 0xD1B54A35u));
				const float ChamberDistance =
					static_cast<float>(FVector::Distance(Query, Node)) - Settings.CaveChamberRadius * ChamberVariation;
	...
	const float WallWarp = SampleNoise3D(static_cast<float>(Query.X) + 1871.0f, static_cast<float>(Query.Y) - 953.0f,
										 static_cast<float>(Query.Z) + 421.0f, Settings.CavePrimaryFrequency) *
							   0.65f +
						   SampleNoise3D(static_cast<float>(Query.X) - 3167.0f, static_cast<float>(Query.Y) + 2297.0f,
										 static_cast<float>(Query.Z) - 811.0f, Settings.CaveSecondaryFrequency) *
							   0.35f;

	return (MinimumSignedDistance - WallWarp * Settings.CaveWallWarpStrength) * Settings.CaveSurfaceSharpness;
}
