...
FCubusDensitySample FCubusTerrainDensityField::SampleContinuous(const FVector& GlobalSampleCoordinate) const
{
	...
	const float TerrainDensity = Column.SurfaceSampleZ - static_cast<float>(GlobalSampleCoordinate.Z);
	...
	{
		Result.MaterialId = Settings.BiomeSettings.bEnabled
								? Settings.BiomeRockMaterialId
								: Settings.RockMaterialId;
	}
	else
	{
		Result.MaterialId = Settings.BiomeSettings.bEnabled
								? Settings.BiomeSubsurfaceMaterialId
...
float FCubusTerrainDensityField::GetCachedSurfaceVoxelHeight(const float WorldSampleX, const float WorldSampleY) const
{
	...
	const float Height = SampleSurfaceVoxelHeight(WorldSampleX - 0.5f, WorldSampleY - 0.5f);
																					   ...
																					   const float WorldSampleY) const
{
	...
	FColumnData Column;
	Column.SurfaceVoxelHeight = GetCachedSurfaceVoxelHeight(WorldSampleX, WorldSampleY);
	...
	Column.Slope = FMath::Sqrt(GradientX * GradientX + GradientY * GradientY);
	...
	else
	{
		Column.SurfaceMaterialId = Settings.BiomeSettings.bEnabled
									   ? FCubusBiomeField::Sample(WorldSampleX - 0.5f, WorldSampleY - 0.5f, Column.SurfaceVoxelHeight,
																  Column.Slope, Settings.BiomeSettings)
											 .SurfaceMaterialId
...
float FCubusTerrainDensityField::ApplyRiverLowering(const float SurfaceHeight, const float WorldX, const float WorldY) const
{
	...
	const float Lowering = Settings.RiverValleyDepth * ValleyInfluence + (bInsideChannel ? Settings.RiverChannelDepth : 0.0f);
												   ...
												   const float SurfaceSlope) const
{
	...
	const float SecondaryNoise =
		FMath::Abs(SampleNoise3D(WorldX + 1871.0f, WorldY - 953.0f, ShiftedWorldZ + 421.0f, Settings.CaveSecondaryFrequency));
