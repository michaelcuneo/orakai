...
struct ORAKAI_API FCubusTerrainFormSettings
{
	...
	float MountainReliefMeters	  = 2400.0f;
...
{
public:
	static FCubusTerrainFormSample Sample(float WorldX, float WorldY, const FCubusTerrainFormSettings& Settings);

private:
	static float SampleNoise(float WorldX, float WorldY, float Frequency);

	static float SampleFbm(float WorldX, float WorldY, float Frequency, int32 Octaves, float Lacunarity, float Gain);

	static float SampleRidgedFbm(float WorldX, float WorldY, float Frequency, int32 Octaves);

	static float SampleChannelMask(float WorldX, float WorldY, float Frequency, float Width, float Falloff);
