...
namespace CubusDensityMesher
{
    ...
    {
        const int32 Slot = Palette.FindSlot(ClampDensityMaterialId(Vertex.MaterialId));
        switch (Slot)
        {
    ...
    public:
        FAdaptiveSampleCache(
            ...
            , GlobalSampleOrigin(
                ToVector(
                    InChunkCoordinate * Cubus::ChunkSize
                )
            )
            , SampleSpacing(
                1.0f /
                static_cast<float>(InSubdivisions)
            )
            , FineChunkSize(
                Cubus::ChunkSize * InSubdivisions
            )
        {
        }
    ...
    )
    {
        ...
        for (int32 CornerIndex = 1; CornerIndex < 8; ++CornerIndex)
        {
            ...
            if (
                SampleCache.GetSample(FineCorner).IsSolid(IsoLevel) !=
                bFirstCornerSolid
            )
            {
                return true;
    ...
    )
    {
        ...
        Result.LocalPosition =
            ChunkMinimum +
            LocalSamplePosition * CanonicalVoxelSize;
    ...
    )
    {
        ...
        for (const FInterpolatedVertex& Vertex : Vertices)
        {
            ...
            MeshData.VertexColors.Add(BuildWeights(Vertex, Palette));
                    ...
                    for (int32 SubY = 0; SubY < Subdivisions; ++SubY)
                    {
                        for (int32 SubX = 0; SubX < Subdivisions; ++SubX)
                        {
                            ...
                            )
                            {
                                ...
                                if (
                                    CornerSamples[CornerIndex].IsSolid(
                                        IsoLevel
                                    )
                                )
                                {
