...
TAutoConsoleVariable<int32> CVarEnabled(TEXT("Cubus.RayTracing.Enabled"), 0,
										TEXT("Enable stable near-field ray-tracing proxy meshes for Cubus terrain."),
										ECVF_Scalability | ECVF_RenderThreadSafe);

TAutoConsoleVariable<int32> CVarHorizontalRadius(TEXT("Cubus.RayTracing.Radius"), 1,
												 TEXT("Horizontal chunk radius covered by stable Cubus ray-tracing proxies."),
												 ECVF_Scalability | ECVF_RenderThreadSafe);

TAutoConsoleVariable<int32> CVarVerticalRadius(TEXT("Cubus.RayTracing.VerticalRadius"), 0,
											   TEXT("Vertical chunk radius covered by stable Cubus ray-tracing proxies."),
											   ECVF_Scalability | ECVF_RenderThreadSafe);

TAutoConsoleVariable<int32> CVarStableChecks(TEXT("Cubus.RayTracing.StableChecks"), 2,
											 TEXT("Number of unchanged manager checks required before a proxy is built."),
											 ECVF_Scalability);

TAutoConsoleVariable<int32> CVarMaxBuildsPerUpdate(TEXT("Cubus.RayTracing.MaxProxyBuildsPerUpdate"), 1,
...
{
	const double ChunkWorldSize = static_cast<double>(Cubus::ChunkSize) * static_cast<double>(FMath::Max(1.0f, VoxelSize));

	return FIntVector(FMath::FloorToInt(ViewLocation.X / ChunkWorldSize), FMath::FloorToInt(ViewLocation.Y / ChunkWorldSize),
					  FMath::FloorToInt(ViewLocation.Z / ChunkWorldSize));
}

bool IsInsideRadius(const FIntVector& Coordinate, const FIntVector& Centre, const int32 HorizontalRadius, const int32 VerticalRadius)
{
	return FMath::Abs(Coordinate.X - Centre.X) <= HorizontalRadius && FMath::Abs(Coordinate.Y - Centre.Y) <= HorizontalRadius &&
...
bool BuildProxy(UWorld& World, ACubusPCGVoxelVolumeActor& SourceChunk, FChunkProxyState& State)
{
	...
	ACubusTerrainRayTracingProxyActor* Proxy = World.SpawnActor<ACubusTerrainRayTracingProxyActor>(
		ACubusTerrainRayTracingProxyActor::StaticClass(), SourceChunk.GetActorTransform(), SpawnParameters);
...
void UpdateWorld(UWorld* World)
{
	...
	const bool bHasView = IsValid(PlayerController) && IsValid(PlayerController->PlayerCameraManager);

	const FVector ViewLocation = bHasView ? PlayerController->PlayerCameraManager->GetCameraLocation()
										  : (IsValid(PlayerPawn) ? PlayerPawn->GetActorLocation() : FVector::ZeroVector);

	const bool bEnabled = CVarEnabled.GetValueOnGameThread() != 0 && (bHasView || IsValid(PlayerPawn));

	const int32 HorizontalRadius = FMath::Clamp(CVarHorizontalRadius.GetValueOnGameThread(), 0, 4);

	const int32 VerticalRadius = FMath::Clamp(CVarVerticalRadius.GetValueOnGameThread(), 0, 2);

	const int32 RequiredStableChecks = FMath::Clamp(CVarStableChecks.GetValueOnGameThread(), 1, 20);

	const int32 MaxBuilds = FMath::Clamp(CVarMaxBuildsPerUpdate.GetValueOnGameThread(), 0, 8);
	...
	for (TActorIterator<ACubusPCGVoxelVolumeActor> Iterator(World); Iterator; ++Iterator)
	{
		...
		UProceduralMeshComponent* SourceMesh = Cast<UProceduralMeshComponent>(Chunk->GetRootComponent());

		const FCubusBlockChunkData* ChunkData = Chunk->GetChunkData();

		if (!IsValid(SourceMesh) || SourceMesh->GetNumSections() <= 0 || ChunkData == nullptr || !ChunkData->HasAnyOccupiedVoxel())
		{
			continue;
		}

		const FIntVector PlayerChunk = ResolveViewChunk(ViewLocation, Chunk->GetVoxelSize());

		if (!IsInsideRadius(Chunk->GetChunkCoordinate(), PlayerChunk, HorizontalRadius, VerticalRadius))
		{
			continue;
		}
		...
		State.StableChecks = FMath::Min(State.StableChecks + 1, RequiredStableChecks);

		ACubusTerrainRayTracingProxyActor* ActiveProxy = State.ActiveProxy.Get();

		if (IsValid(ActiveProxy) && !ActiveProxy->IsRetiring() && ActiveProxy->GetSourceRevision() == CurrentRevision)
		{
			continue;
		}
		...
		if (State.StableChecks >= RequiredStableChecks && BuildsThisUpdate < MaxBuilds && BuildProxy(*World, *Chunk, State))
		{
			++BuildsThisUpdate;
		}
	}
	...
	for (auto Iterator = WorldState.Chunks.CreateIterator(); Iterator; ++Iterator)
	{
		...
		if (!IsValid(SourceChunk) || !DesiredChunks.Contains(Iterator.Key()))
		{
...
bool Tick(const float DeltaTime)
{
	...
	{
		if (!Iterator.Key().IsValid() || !ActiveWorlds.Contains(Iterator.Key()))
		{
...
{
	FRegistration() { TickerHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateStatic(&Tick), UpdateIntervalSeconds); }
...
void ACubusPCGVoxelVolumeActor::SetTerrainRayTracingEnabled(const bool bEnabled)
{
	...
	UProceduralMeshComponent* Mesh = Cast<UProceduralMeshComponent>(GetRootComponent());
